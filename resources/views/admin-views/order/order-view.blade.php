                    ...
                    <div class="card-header flex-wrap align-items-start border-0">
                        ...
                        <div class="w-100">
                            ...
                            @if($order['bring_change_amount'])
                                <h5><strong>{{ translate('Bring Change Note') }}</strong>: <span class="badge-soft-success">{{translate('Please ensure the Deliveryman has '). \App\CentralLogics\Helpers::set_symbol($order['bring_change_amount']) . translate(' in change ready for the customer')}}</span></h5>
                            @endif
                        </div>
                    </div>
                                ...
                                @if($order->offline_payment)
                                    ...
                                    @if($order->offline_payment?->status == 0)
                                        <div class="d-flex flex-wrap justify-content-center gap-3 mt-4">
                                            ...
                                            <a class="btn badge-info flex-grow-1 text-info bg-opacity-info-10" href="{{route('admin.orders.verify-offline-payment', ['order_id' => $order->id, 'status' => 1])}}">{{ translate('approve') }}</a>
                                        </div>
                                    @elseif($order->offline_payment?->status == 2)
                                        @if($order['order_status'] != 'canceled' && $order['order_status'] != 'failed' && $order['order_status'] != 'return' && $order['order_status'] != 'delivered')
                                            <div class="d-flex flex-column gap-2 mt-4">
                                                <a class="btn btn--primary w-100" href="{{ route('admin.orders.switch-payment',['id'=>$order['id']]) }}">{{ translate('Switch to Cash On Delivery') }}</a>
                                                <a class="btn btn-danger w-100 bg-opacity-danger-10 text-danger" href="{{ route('admin.orders.status',['id'=>$order['id'],'order_status'=>'canceled']) }}">{{ translate('Cancel Order') }}</a>
                                            </div>
                                        @endif
                                    @endif
                                @else
    ...
    <div class="modal fade" id="deniedModal-{{$order->id}}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog">
          ...
          <span aria-hidden="true">&times;</span>
                    ...
                    <form method="GET" action="{{route('admin.orders.verify-offline-payment',['order_id' => $order->id, 'status' => 2])}}">
                        <div>
