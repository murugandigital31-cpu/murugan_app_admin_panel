                    ...
                    <tr>
                        ...
                        <td class="align-items-center text-center">
                            <input type="number" data-key="{{$key}}" class="amount--input form-control text-center" id="{{ $cartItem['id'] }}"
                                   value="{{$cartItem['quantity']}}" min="1" max="{{ $cartItem['total_stock'] }}" onkeyup="updateQuantity(event)">
                        </td>
...
<div class="box p-3">
    <dl class="row">
        ...
        @php($totalOrderAmount = $total+$updatedTotalTax+$delivery_charge+$productWeightCharge)
        <dt class="col-sm-6">{{translate('total')}} :</dt>
        <dd class="col-sm-6 text-right h4 b">{{ Helpers::set_symbol(round($totalOrderAmount, 2)) }}</dd>
    </dl>
    <div>
        <form action="{{route('branch.pos.order')}}" id='order_place' method="post">
            ...
            <div class="pos--payment-options mt-3 mb-3">
                ...
                <ul>
                    <li style="display: {{ !session()->has('order_type') || session('order_type') == 'take_away' ?  'block' : 'none' }}">
                        <label>
                            <input type="radio" class="paid-by" name="type" value="cash" hidden="" {{ !session()->has('order_type') || session('order_type') == 'take_away' ? 'checked' : '' }}>
                    ...
                    <li style="display: {{ session('order_type') == 'home_delivery' ?  'block' : 'none' }}">
                        <label>
                            <input type="radio" class="paid-by" name="type" value="cash_on_delivery" hidden="" {{ session('order_type') == 'home_delivery' ? 'checked' : '' }}>
            ...
            <div class="collect-cash-section" style="display: {{ session('order_type') != 'home_delivery' ?  'block' : 'none' }}">
                <div class="form-group mb-2 d-flex align-items-center justify-content-between gap-2">
                    ...
                    <input type="number" class="form-control w-50 text-right" name="paid_amount"
                           step="0.01" id="paid-amount" min="0"
                           value="{{ round($totalOrderAmount, 2) }}"
                           onkeyup="calculateAmountDifference()"
                           required>
            ...
            <div class="row button--bottom-fixed g-1 bg-white ">
                ...
                <div class="col-sm-6">
                    <span id="place-order-tooltip-wrapper">
                        <button type="submit"
                                class="btn btn--primary btn-sm btn-block order-place-btn">
