...
class PasswordResetController extends Controller
{
    ...
    public function resetPasswordRequest(Request $request): JsonResponse
    {
        ...
        if (!isset($customer)) {
            return response()->json(['errors' => [['code' => 'not-found', 'message' => 'Customer not found!']]], 404);
        }
        ...
        $emailStatus = isset($emailConfig['status']) ? (int)$emailConfig['status'] : 0;

        if ($request['type'] === 'phone' && $firebaseOTPVerificationStatus == 0 && $activeCount == 0) {
            return response()->json(['errors' => [['code' => 'phone', 'message' => translate('Something went wrong. Kindly contact with admin.')]]], 403);
        }
        if ($request['type'] == 'email' &&  $emailStatus == 0){
            return response()->json(['errors' => [['code' => 'email', 'message' => translate('Something went wrong. Kindly contact with admin.')]]], 403);
        }
        ...
        if ($request['type'] == 'phone') {
            ...
            if(addon_published_status('Gateways')){
                $response = SmsGateway::send($request['email_or_phone'],$token);
            }else{
                $response = SMS_module::send($request['email_or_phone'], $token);
            }
