...
class OrderController extends Controller
{
    ...
    public function placeOrder(Request $request): \Illuminate\Http\JsonResponse
    {
        ...
        $calculateDeliveryCharge = Helpers::get_delivery_charge(branchId: $request['branch_id'], distance:  $request['distance'], selectedDeliveryArea: $request['selected_delivery_area']);

        if ($request['order_type'] == 'self_pickup'){
            $deliveryCharge = 0;
        } elseif (Helpers::get_business_settings('free_delivery_over_amount_status') == 1 && (Helpers::get_business_settings('free_delivery_over_amount') <= $request['order_amount'] - $calculateDeliveryCharge)){
            $deliveryCharge = 0;
            $freeDeliveryAmount = $calculateDeliveryCharge;
        } else{
